...
# python -m unittest -v  test_workout_views.py #
print("------------------")
print("WORKOUT VIEWS TESTS")
print("------------------")
...
class TeamViewTestCase(TestCase):
    ...
    def test_workouts_show_logged_out(self):
        """ Test: can a logged-out user see all the workouts?"""
        with self.client as c:

            resp = c.get(f"/workouts")

            self.assertEqual(resp.status_code, 200)
            self.assertIn('<h1 class="d-inline teams-fa-color display-5 px-1">Workouts</h1>', str(resp.data))
        
    
    def test_workouts_show_logged_in(self):
        """ Test: can a logged-in user see all the workouts?"""
        with self.client as c:
    ...
    def test_workout_show_logged_in(self):
        """ Test: can a logged-out user see a specific workout? """
        with self.client as c:
            with c.session_transaction() as sess:
                sess[CURR_USER_KEY] = self.user_id
           
            self.workout = self.workouts[0]
            self.workout_id = self.workout.id
            
            resp = c.get(f"/workouts/{self.workout_id}")
       
            self.assertEqual(resp.status_code, 200)
            self.assertIn(f'<h5 class="card-link fs-4 ps-3 pt-2">Upper Body Strength</h5>', str(resp.data))
    
    
    def test_workout_show_logged_out(self):
        """ Test: can a logged-out user see a specific workout? """
        with self.client as c:
    ...
    def test_workouts_add_logged_in(self):
        """when logged-in, can the user add a workout?"""

        with self.client as c:
            with c.session_transaction() as sess:
                sess[CURR_USER_KEY] = self.user.id

            resp = c.post(f"/workouts/add", data={ "name": "testname",
                        "description": "testdescription"} )

            self.assertEqual(resp.status_code, 200)
            workout = Workout.query.get_or_404(3)
            self.assertEqual(workout.name, "testname")     

    def test_workouts_add_logged_out(self):
        """when logged-in, can the user add a workout?"""
        with self.client as c:
            resp = c.post(f"/workouts/add", data={ "name": "testname",
                        "description": "testdescription"} ,follow_redirects=True )

            self.assertEqual(resp.status_code, 200)
            self.assertIn(f'href="/users/register">REGISTER NOW!</a>', str(resp.data))     

    
    def test_workouts_edit(self):
        """when logged-in, can the user edit a workout?"""
        ...
        resp = c.post(f"/workouts/edit/{self.workout_id}", data={ "name": "testname1",
                        "description": "testdescription"}  )

        self.assertEqual(resp.status_code, 200)

    def test_workouts_delete(self):
        """when logged-in, can the user delete a workout?"""
        ...
        resp = c.post(f"/workouts/delete/{self.workout_id}", follow_redirects=True )
